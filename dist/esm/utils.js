 ...
 */
export function colorize(str, ...styles) {
 ...
 */
export function pad(str, dir, spaces = 0) {
    if (dir === 'left')
 ...
 */
export function truncate(str, max = 0, char = '...') {
    if (!max || !str)
        return str;
    if (str.length <= max)
        return str;
 ...
 */
export function cloneClass(instance) {
 ...
 */
export function ensureDefaults(options, defaults) {
    const run = (o, d) => {
        for (const k in d) {
