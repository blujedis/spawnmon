...
const wrapAnsi$1 = code => `${ESCAPES$1.values().next().value}${ANSI_CSI}${code}${ANSI_SGR_TERMINATOR}`;
...
const wrapWord = (rows, word, columns) => {
	...
	let visible = stringWidth_1(stripAnsi$1(rows[rows.length - 1]));
...
function pinger(port, host) {
    ...
    function dispatch(event) {
        if (_events[event].length)
            _events[event].forEach((function (handler) { return handler(_tries, _client); }));
    }
    function connect(options) {
        if (options === void 0) { options = {}; }
        return __awaiter(this, void 0, void 0, function () {
            var retries, delay, rest;
            return __generator(this, function (_a) {
                ...
                retries = options.retries, delay = options.delay, rest = __rest(options, ["retries", "delay"]);
                _retries = retries;
                _tries += 1;
                dispatch('retry');
                _client = new net.Socket(rest);
                _client.connect({ host: host, port: port }, function () {
                    _client.end();
                    dispatch('connected');
                });
...
var NEWLINE_EXP = /\r?\n$/u;
var symbols = {
    alert: logSymbols.error,
    caution: logSymbols.warning,
    notice: logSymbols.info,
 ...
 */
function chomp(line) {
    var match = NEWLINE_EXP.exec(line);
    if (!match)
        return line;
 ...
 */
function wrapAnsi(line, columns, options) {
    return wrapAnsi_1(line, columns, options);
}
/**
...
function colorize(str) {
    ...
    return styles.reduce(function (result, color) {
        if (!ansiColors[color])
            return result;
        return ansiColors[color](result);
    }, str);
}
/**
 ...
 */
function getLongest(values, strip) {
 ...
 */
function truncate(str, max, char) {
    if (max === void 0) { max = 0; }
    if (char === void 0) { char = '...'; }
    if (!max || !str)
        return str;
 ...
 * @returns
 */
function padInspect(values, buffer, strip) {
    if (buffer === void 0) { buffer = 2; }
    if (strip === void 0) { strip = true; }
    var longest = getLongest(values, strip);
    buffer = buffer || 0;
    return values.map(function (str) {
        var stripped = stripAnsi(str);
        var adj = longest - stripped.length;
        return [str, buffer ? adj + buffer : adj];
    });
}
function getRandomValue(collection, asPosition) {
    var random = Math.floor(Math.random() * collection.length);
    if (asPosition)
        return random;
    return collection[random];
}
function getRandomValues(collection, count, asPosition) {
    ...
    function run(unused) {
        ...
        var _loop_1 = function () {
            var random = getRandomValue(collection, asPosition);
            if (!found.includes(random)) {
                found.push(random);
                unused = unused.filter(function (v) { return v !== random; });
            }
        };
    ...
    run(__spreadArray([], __read(collection)));
    return found;
}
/**
 ...
 */
function defaultWriteHandler$1(line) {
 ...
 */
function writeLines$1(readable, transform) {
    var e_2, _a;
    if (transform === void 0) { transform = defaultWriteHandler$1; }
    return __awaiter(this, void 0, void 0, function () {
        var _b, _c, line, str, e_2_1;
        return __generator(this, function (_d) {
            switch (_d.label) {
                case 0:
                    _d.trys.push([0, 5, 6, 11]);
                    _b = __asyncValues(chunksToLines(readable));
                    _d.label = 1;
...
function exceptionHandler(err) {
    process.stderr.write(ansiColors.redBright(err.stack || err.message || err + '\n'));
}
/**
 ...
 */
function registerExceptions(handler) {
    if (handler === void 0) { handler = exceptionHandler; }
    var uncaught = registerEvent('uncaughtException', handler);
    var rejected = registerEvent('unhandledRejection', exceptionHandler);
    return function () {
        uncaught();
        rejected();
    };
}
function registerEvent(signalOrEvent, handler) {
    process.on(signalOrEvent, handler);
    return function () {
...
var utils = {
    ...
    registerExceptions: registerExceptions,
    writeLines: writeLines$1,
    defaultWriteHandler: defaultWriteHandler$1,
    pad: pad,
    padInspect: padInspect,
...
var writeLines = utils.writeLines, defaultWriteHandler = utils.defaultWriteHandler;
var _baseDefaults = {
    stdio: [process.stdin, 'pipe', process.stderr],
    writestream: 'stdout'
};
function createRunner(defaultTransform, defaultSpawnOptions) {
    if (defaultTransform === void 0) { defaultTransform = defaultWriteHandler; }
    if (defaultSpawnOptions === void 0) { defaultSpawnOptions = __assign({}, _baseDefaults); }
    if (typeof defaultTransform === 'object' && defaultTransform !== null) {
        defaultSpawnOptions = defaultTransform;
        defaultTransform = undefined;
    }
    defaultSpawnOptions = defaultSpawnOptions || {};
    var _processes = [];
    var _commands = [];
    var _delays = []; // temp hack.
    var api = {
     ...
     */
    function toTuple(cmd, args, opts, transform) {
        ...
        args = args || [];
        opts = __assign(__assign({}, defaultSpawnOptions), (opts || {}));
        return [cmd, args, opts, transform];
    }
    // Common.
    /**
    ...
    function kill() {
        ...
        for (var _i = 0; _i < arguments.length; _i++) {
            pids[_i] = arguments[_i];
        }
        if (!pids.length)
            pids = api.processes.map(function (proc) { return proc.pid; });
    ...
    function add(cmd, args, opts, transform) {
        var conf = toTuple(cmd, args, opts, transform);
        api.commands.push(conf);
        var idx = api.commands.indexOf(conf);
        // Create a new api.
        var _api = Object.create(api);
        _api.options = {};
        ...
        _api.run = function (tform) {
            // user may have added transform add to conf if found.
            if (!conf[3]) {
                tform = tform || _api.options.transform;
                conf[3] = tform;
            }
            _api.options.child = run.apply(void 0, __spreadArray([], __read(conf)));
            return _api;
        };
        _api.delay = function (delay) {
            if (delay === void 0) { delay = 1000; }
            _api.options.delay = delay;
            _delays[idx] = delay;
            return _api;
        };
        _api.transform = function (handler) {
    ...
    function createMonitor(tick, cb) {
        var ctr = 0;
        var prevCtr = 0;
        var timeoutId;
        var running = false;
    ...
    function run(initCmd, argsOptsOrHandler, optsOrHandler, initTransform) {
        var _this = this;
        // normalize all of our options.
        var _a = __read(toTuple(initCmd, argsOptsOrHandler, optsOrHandler, initTransform), 4), cmd = _a[0], args = _a[1], opts = _a[2], transform = _a[3];
        ...
        if (opts.writestream === 'stderr') {
            opts.stdio = [stdio[0], 'inherit', 'pipe'];
        }
        else if (opts.writestream === 'stdout') {
            opts.stdio = [stdio[0], 'pipe', stdio[2]];
        }
        ...
        var stream = opts.writestream === 'stderr' ? child.stderr : child.stdout;
        // create monitor.
        var mon = createMonitor(2500);
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        if (!mon.running)
                            mon.start();
                        return [4 /*yield*/, writeLines(stream, transform)];
                    case 1:
                        _a.sent();
                        return [2 /*return*/];
                }
    ...
    function runAfter(delay, command, args) {
        if (!delay)
            return command.apply(void 0, __spreadArray([], __read(args)));
        setTimeout(function () {
     ...
     */
    function runAll(confs) {
        if (confs === void 0) { confs = api.commands; }
        confs.forEach(function (conf) {
            var isSync = conf.length === 5;
            var idx = api.commands.indexOf(conf);
            var delay = _delays[idx];
            if (isSync) {
                conf.pop();
