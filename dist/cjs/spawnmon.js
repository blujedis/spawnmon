...
class Spawnmon {
    ...
    getPrefix(command, color = this.options.defaultColor) {
        ...
        // and pad if needed.
            ...
            if (indexesLen === 1 && label.length <= innerLength) {
                ...
                prefix = utils_1.truncate(label, innerLength, '');
    ...
    run(...commands) {
        ...
        const cmds = commands.map(cmd => {
            ...
            if (groups && groups.length) {
                ...
                this.groups[exports.DEFAULT_GROUP_NAME] = this.groups[exports.DEFAULT_GROUP_NAME] || [];
                this.groups[exports.DEFAULT_GROUP_NAME].push(cmd);
            }
            return cmd;
        });
        this.indexes = [...cmds];
    ...
    runGroup(...groups) {
        if (!groups.length)
            throw utils_1.createError(`At least one group must be specified to run groups.`);
        const commands = groups.reduce((a, c) => {
            if (!this.hasGroup(c))
                throw utils_1.createError(`Failed to lookup commands for unknown group ${c}.`);
    ...
    handleInput(command) {
        const cmd = this.get(command);
        const subscription = rxjs_1.fromEvent(process.stdin, 'data')
            .pipe(operators_1.map(data => data.toString()))
            .subscribe(data => {
            const name = (cmd && cmd.name) || 'anonymous';
            if (!cmd || !cmd.stdin)
                throw utils_1.createError(`Cannot pipe input stream, command ${name} unknown or has not stdin currently open.`);
